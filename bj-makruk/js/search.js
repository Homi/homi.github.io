...
function UpdateDOMStats() {
		...
		var tempHalfMove = MATE - Math.abs(domUpdate_score);
		
		if(Math.abs(domUpdate_score) > MATE-MAXDEPTH) {
			scoreText = "Score: " + "Mate In " + ((tempHalfMove-1)/2).toFixed(0) + " Moves";
		}
...
function SearchPosition() {
	...
	for( currentDepth = 1; currentDepth <= srch_depth; ++currentDepth ) {						
		...
		if( Math.abs( bestScore ) >= (INFINITE - MAXDEPTH) - currentDepth - 3 ) break; // CheckMate, don't search further
	}	
